...
services:
  ...
  # Hourly auto-upgrade: pulls the latest :stable image and restarts my-node
  # only when the image actually changes. Built on Docker's official CLI image,
  # so there is no third-party dependency to maintain, and scoped to `my-node`
  # so it never recreates itself.
  updater:
    image: docker:cli
