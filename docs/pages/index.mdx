...
:::note
**Terminology note:** the Open Audio Protocol uses a single unified node type called the **Open Audio Validator Node**. Earlier versions of Audius used separate *Discovery Nodes* and *Content Nodes* — those terms are deprecated and both roles are now served by validator nodes. See [Terminology](/concepts/terminology) for the full mapping.
:::