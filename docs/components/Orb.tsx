...
function Orb({ className, style, onReady }: OrbProps) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const canvasEl = app.canvas as HTMLCanvasElement | null
      ...
      const handlePointerMove = (e: PointerEvent) => {
        ...
        const clampedX = Math.min(rect.right, Math.max(rect.left, e.clientX)) - rect.left
        const clampedY = Math.min(rect.bottom, Math.max(rect.top, e.clientY)) - rect.top
        pointerX = clampedX
        pointerY = clampedY
      ...
      const VIS_BINS = 96
      const repeats = 18
      const ringSmoothing = 0.6
      const audioMaxOffsetRatio = 0.12
      ...
      const physics = {
        ...
        tetherLenXRatio: 0.75,
        tetherLenYRatio: 0.32,
      }
      ...
      const computeTargetRadius = (idx: number): number => {
        ...
        const overNorm = Math.min(1, Math.max(0, over / Math.max(1e-6, edgeRadius * 0.45)))
        const overSmooth = overNorm * overNorm * (3 - 2 * overNorm)
        if (!pointerActive || overSmooth <= 0) {
          return effectiveBaseRadius
        }
        const gauss = Math.exp(-(dTheta * dTheta) / (2 * physics.attractionSigma * physics.attractionSigma))
        const maxDeform = 0.06
        const influence = Math.min(maxDeform, physics.attractionStrength * gauss * overSmooth)
        return effectiveBaseRadius * (1 - influence)
      }
      ...
      const drawCircle = () => {
        ...
        // Axis-wise non-linear tether using tanh per-axis with wider horizontal pull
        const tetherLenXRatio = (physics as any).tetherLenXRatio !== undefined ? (physics as any).tetherLenXRatio : physics.tetherLengthRatio
        const tetherLenYRatio = (physics as any).tetherLenYRatio !== undefined ? (physics as any).tetherLenYRatio : physics.tetherLengthRatio
        const tetherLenX = Math.max(1, width * tetherLenXRatio)
        const tetherLenY = Math.max(1, height * tetherLenYRatio)
        const offXToTarget = tetherLenX * Math.tanh(dx / Math.max(1e-6, tetherLenX))
        const offYToTarget = tetherLenY * Math.tanh(dy / Math.max(1e-6, tetherLenY))
        ...
        // Neighbor-smoothed target radii
        const baseTargets: number[] = new Array(NUM_POINTS)
        for (let i = 0; i < NUM_POINTS; i++) baseTargets[i] = computeTargetRadius(i)
        const SMOOTH_PASSES = 3
        const SMOOTH_K = 0.36
        let targets: number[] = baseTargets.slice()
        for (let p = 0; p < SMOOTH_PASSES; p++) {
          const prev = targets.slice()
          for (let i = 0; i < NUM_POINTS; i++) {
            const left = prev[(i - 1 + NUM_POINTS) % NUM_POINTS]
            const right = prev[(i + 1) % NUM_POINTS]
            targets[i] = prev[i] * (1 - SMOOTH_K) + 0.5 * (left + right) * SMOOTH_K
          }
        }

        for (let i = 0; i < NUM_POINTS; i++) {
          let targetR = targets[i]
          if (visValues) {
      ...
      ensureAudioElement()
      drawCircle()
      // Fire onReady once after first frame to enable external fade-in
      try {
        requestAnimationFrame(() => {
          if (!readyFiredRef.current) {
            readyFiredRef.current = true
            try { onReadyRef.current?.() } catch { /* ignore */ }
          }
        })
      } catch { /* ignore */ }
